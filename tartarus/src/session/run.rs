...
pub struct RunRequest {
    ...
    /// GPU passthrough request from `--gpu` (`None`, `"auto"`, or a BDF).
    pub gpu: Option<String>,

    /// `--privileged-libvirt`: use `qemu:///system` instead of
    /// `qemu:///session` (weaker isolation).
...
fn enforce_gpu_pre_check(request: &RunRequest) -> Result<Option<crate::gpu::PciAddress>> {
    ...
    let target = if spec == "auto" {
        match crate::gpu::pci::pick_auto_gpu()? {
            ...
            None => {
                return Err(crate::error::Error::Gpu(crate::gpu::GpuError::NoCleanGpuFound));
            },
        }
    } else {
        spec.parse()?
...
fn prepare_gpu_borrow(request: &RunRequest) -> Result<Option<GpuBundle>> {
    ...
    if let Some(holder) = crate::session::gpu_index::find_borrowing_session(&target)? {
        return Err(crate::error::Error::Session(
            crate::session::error::SessionError::AliasInUse {
...
fn build_seed(config: &Config, alias: Option<&str>, inputs: SeedInputs) -> Result<Seed> {
    ...
    Seed::from_config(config, alias, inputs)?.ok_or_else(|| Error::from(SessionError::MissingCredentials))
}
...
mod tests {
    ...
    fn enforce_gpu_pre_check_rejects_malformed_bdf() {
        ...
        assert!(
            matches!(
                err,
                crate::error::Error::Gpu(crate::gpu::GpuError::InvalidPciAddress { .. })
            ),
    ...
    fn sample_config() -> Config {
        Config {
            ...
            base_repos: vec![],
            claude_backend: crate::config::Backend::Anthropic,
