...
pub fn run(config: &Config, target: &str) -> Result<GrowOutcome> {
    ...
    let info = read_qemu_img_info(&overlay)?;
    ...
    qemu_img_resize(&overlay, after_gib)?;
...
/// Run `qemu-img info --output=json` against `path`.
fn read_qemu_img_info(path: &Path) -> Result<QemuImgInfo> {
    let output = Command::new("qemu-img")
        .arg("info")
        .arg("--output=json")
        .arg(path)
        .output()
        .map_err(|err| GrowError::QemuImg {
            detail: err.to_string(),
            operation: "info",
            path: path.to_path_buf(),
        })?;
...
/// Run `qemu-img resize <path> <gib>G`.
fn qemu_img_resize(path: &Path, new_size_gib: u32) -> Result<()> {
    let args = resize_args(path, new_size_gib);
    let output = Command::new("qemu-img")
        .args(&args)
        .output()
        .map_err(|err| GrowError::QemuImg {
