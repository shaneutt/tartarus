...
fn genisoimage_build(workdir: &Path, iso_path: &Path) -> Result<()> {
    crate::seed::iso::run_genisoimage(workdir, iso_path)
}
...
///
/// Thin wrapper around [`crate::time::today_iso`] kept here because
/// the only caller is the base-pull file-name composer.
fn today_iso() -> String {
    crate::time::today_iso()
}
...
mod tests {
    ...
    fn verify_image_against_manifest_rejects_tampered_image() {
        ...
        assert!(matches!(
            err,
            crate::error::Error::Base(BaseError::ChecksumMismatch { .. }),
        ));
    }
