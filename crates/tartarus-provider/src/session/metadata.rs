...
//!
//! Provider-specific fields (libvirt's GPU borrow record, the SSH
//! loopback port) are carried as serialisable plain data; converting
//! [`GpuBorrowRecord`] to a typed receipt is the libvirt provider's
...
use crate::{error::Result, seed::input::RepoSpec, session::SessionError};
...
///
/// Reconstructing a typed receipt from this record is the libvirt
/// provider's responsibility; this crate only stores the on-disk
/// representation.
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
...
mod tests {
    ...
    fn load_rejects_below_min_version() {
        ...
        match err {
            crate::Error::Session(SessionError::MetadataVersion { version }) => {
    ...
    fn load_returns_typed_error_on_bad_json() {
        ...
        match err {
            crate::Error::Session(SessionError::MetadataParse { .. }) => {},
