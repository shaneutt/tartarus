...
pub struct Config {
    ...
    /// Provider backend selector: `libvirt` (default) or `hetzner`.
    pub provider: ProviderKind,

    /// Hetzner-specific settings; required when `provider = "hetzner"`.
    pub hetzner: Option<HetznerSection>,
}
...
/// Which session provider the binary should dispatch through.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq, Deserialize, Serialize)]
#[serde(rename_all = "lowercase")]
pub enum ProviderKind {
    /// libvirt + QEMU/KVM (default).
    #[default]
    Libvirt,
...
/// `[hetzner]` section of the TOML config.
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
pub struct HetznerSection {
    ...
    /// Optional Hetzner project SSH-key name for fallback access.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub ssh_key_name: Option<String>,

    /// Per-session `/home` volume size in GiB. `0` (default) skips
    /// the volume.
    #[serde(default)]
    pub volume_gib: u32,
}
...
pub struct FileConfig {
    ...
    /// `[hetzner]` section, only meaningful when `provider = "hetzner"`.
    pub hetzner: Option<HetznerSection>,
}
