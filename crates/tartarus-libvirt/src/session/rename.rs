...
fn update_metadata_alias(session_dir: &std::path::Path, alias: &str) -> Result<()> {
    let path = session_dir.join(tartarus_provider::session::metadata::METADATA_FILE_NAME);
    let mut metadata = tartarus_provider::session::metadata::Metadata::load(&path)?;
    metadata.alias = Some(alias.to_owned());
    Ok(metadata.save(&path)?)
}
...
mod tests {
    ...
    use tartarus_provider::session::metadata::{self, METADATA_FILE_NAME};
