...
use tartarus_provider::{paths, session::metadata::Metadata};
...
pub fn lookup_receipt(address: &PciAddress) -> Result<Receipt> {
    let session =
        find_borrowing_session(address)?.ok_or_else(|| tartarus_provider::session::SessionError::NotFound {
    ...
    crate::gpu::driver::record_into_receipt(record)
}
...
pub fn clear_receipt(address: &PciAddress) -> Result<()> {
    ...
    let metadata_path = paths::sessions_by_uuid_dir()?
        .join(&session.uuid)
        .join(tartarus_provider::session::metadata::METADATA_FILE_NAME);
...
mod tests {
    ...
    use tartarus_provider::session::metadata::{self as md, GpuBorrowRecord};

    use super::*;
    use crate::gpu::driver::Receipt as DriverReceipt;
